...
#define GET_CONFIG(dev) ((const struct sc16is7xx_gpio_config *)(dev)->config)
#define GET_DATA(dev)   ((struct sc16is7xx_gpio_data *)(dev)->data)
...
#define DT_INST_INIT(inst)                                                                         \
	...
	DEVICE_DT_INST_DEFINE(inst, sc16is7xx_gpio_init, NULL, &inst_##inst##_data,                \
			      ...
			      CONFIG_UART_SC16IS7XX_GPIO_INIT_PRIORITY, &api);

DT_INST_FOREACH_STATUS_OKAY(DT_INST_INIT)

BUILD_ASSERT(CONFIG_UART_SC16IS7XX_GPIO_INIT_PRIORITY > CONFIG_UART_SC16IS7XX_INIT_PRIORITY);
