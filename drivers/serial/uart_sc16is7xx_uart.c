...
#define GET_CONFIG(dev) ((const struct sc16is7xx_uart_config *)(dev)->config)
#define GET_DATA(dev)   ((struct sc16is7xx_uart_data *)(dev)->data)
...
static int sc16is7xx_uart_configure_fifo(const struct device *dev)
{
	...
	/*
	 * Sets the trigger level for the RX FIFO to 56 characters
	 */
	WRITE_BIT(GET_DATA(dev)->fcr, SC16IS7XX_FCR_RX_TRIGGER_LSB_pos, 0);
	WRITE_BIT(GET_DATA(dev)->fcr, SC16IS7XX_FCR_RX_TRIGGER_MSB_pos, 1);

	/*
	 * Sets the trigger level for the TX FIFO to 56 spaces
	 */
	WRITE_BIT(GET_DATA(dev)->fcr, SC16IS7XX_FCR_TX_TRIGGER_LSB_pos, 1);
	WRITE_BIT(GET_DATA(dev)->fcr, SC16IS7XX_FCR_TX_TRIGGER_MSB_pos, 1);
					     ...
					     const struct uart_config *cfg)
{
	 ...
	 */
	const int divider1 =
		DIV_ROUND_CLOSEST(GET_CONFIG(dev)->clock_frequency / 1.0, cfg->baudrate * 16.0);
	const int divider4 =
		DIV_ROUND_CLOSEST(GET_CONFIG(dev)->clock_frequency / 4.0, cfg->baudrate * 16.0);
	const int baudrate1 =
		DIV_ROUND_CLOSEST(GET_CONFIG(dev)->clock_frequency, 1.0 * divider1 * 16.0);
	const int baudrate4 =
		DIV_ROUND_CLOSEST(GET_CONFIG(dev)->clock_frequency, 4.0 * divider4 * 16.0);

	/*
	 * Check if the effective baud rate with the extra prescaler enabled is closer to the
	 * desired one
	 */
	if (ABS_DIFF(cfg->baudrate, baudrate4) < ABS_DIFF(cfg->baudrate, baudrate1)) {
		WRITE_BIT(GET_DATA(dev)->mcr, SC16IS7XX_MCR_CLOCK_DIVISOR_pos, 1);
		GET_DATA(dev)->dlh = (uint8_t)divider4 >> 8;
		GET_DATA(dev)->dll = (uint8_t)divider4;
		LOG_INF("Effective baud rate: %d", baudrate4);
	} else {
		WRITE_BIT(GET_DATA(dev)->mcr, SC16IS7XX_MCR_CLOCK_DIVISOR_pos, 0);
		GET_DATA(dev)->dlh = (uint8_t)divider1 >> 8;
		GET_DATA(dev)->dll = (uint8_t)divider1;
		LOG_INF("Effective baud rate: %d", baudrate1);
	}
...
{
	switch (GET_DATA(dev)->iir & SC16IS7XX_IIR_INTERRUPT_msk) {
	case SC16IS7XX_IIR_RLSE_msk:
	case SC16IS7XX_IIR_RTOI_msk:
	case SC16IS7XX_IIR_RHRI_msk:
	case SC16IS7XX_IIR_THRI_msk:
		return 1;
	default:
		return 0;
	}
...
#define DT_INST_INIT(inst)                                                                         \
	...
	DEVICE_DT_INST_DEFINE(inst, sc16is7xx_uart_init, NULL, &inst_##inst##_data,                \
			      ...
			      CONFIG_UART_SC16IS7XX_UART_INIT_PRIORITY, &api);

DT_INST_FOREACH_STATUS_OKAY(DT_INST_INIT)

BUILD_ASSERT(CONFIG_UART_SC16IS7XX_UART_INIT_PRIORITY > CONFIG_UART_SC16IS7XX_INIT_PRIORITY);
